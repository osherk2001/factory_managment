...
import {
  ...
  resolveIssue,
  listProductIssues,
} from "../../src/modules/issues/server";
import {
  recordWeightEvent,
  correctWeightEvent,
  getProductWeightHistory,
} from "../../src/modules/weights/server";
import {
  listOperationalProducts,
  getOperationalDashboard,
} from "../../src/modules/reports/operations.service";
...
const session = (actor: typeof manager) =>
...
describe.sequential("MVP issues, materials and manager operations", () => {
  beforeAll(async () => {
    a = await makeFactory("Operations A");
    b = await makeFactory("Operations B");
    manager = await makeActor(a.id);
    viewer = await makeActor(a.id, ["products.read"]);
    foreign = await makeActor(b.id);
    session(manager);
    product = await createProduct({ idempotencyKey: randomUUID() });
    session(foreign);
    otherProduct = await createProduct({ idempotencyKey: randomUUID() });
  });
  ...
  it("records one issue on concurrent duplicate delivery and rejects changed requests", async () => {
    ...
    await expect(
      createIssue({ ...input, type: "Changed" }),
    ).rejects.toMatchObject({ code: "ISSUE_IDEMPOTENCY_CONFLICT" });
  ...
  it("calculates decimal material totals and preserves correction history", async () => {
    ...
    const [issued, retry] = await Promise.all([
      recordWeightEvent(input),
      recordWeightEvent(input),
    ]);
    expect(retry).toEqual(issued);
    await recordWeightEvent({
      productId: product.id,
      type: "ISSUED",
      grams: "0.200",
      idempotencyKey: randomUUID(),
    });
    const correction = await correctWeightEvent({
      productId: product.id,
      correctsWeightEventId: issued.id,
      grams: "-0.010",
    ...
    expect(history.summary.issued).toBe("0.290");
    expect(history.summary.materialVariance).toBe("0.040");
    expect(history.events.find((e) => e.id === issued.id)?.grams).toBe("0.100");
    await expect(
      correctWeightEvent({
        productId: product.id,
        correctsWeightEventId: correction.id,
        grams: "1",
        note: "Wrong target",
        idempotencyKey: randomUUID(),
      }),
    ).rejects.toMatchObject({ code: "WEIGHT_CORRECTION_TARGET_INVALID" });
    for (const grams of ["0", "-1", "0.0001", "1e3", "NaN", "10000000"])
      await expect(
        recordWeightEvent({ ...input, grams, idempotencyKey: randomUUID() }),
      ).rejects.toMatchObject({ code: "WEIGHT_INVALID_INPUT" });
  });
  it("enforces permissions and foreign resource isolation for issues and weights", async () => {
    ...
    ).rejects.toMatchObject({ code: "ISSUE_PRODUCT_NOT_FOUND" });
    await expect(
      recordWeightEvent({
        productId: otherProduct.id,
        type: "ISSUED",
    ...
    ).rejects.toMatchObject({ code: "WEIGHT_PRODUCT_NOT_FOUND" });
    ...
    await expect(getProductWeightHistory(product.id)).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(listProductIssues(product.id)).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(
      correctWeightEvent({
        productId: product.id,
        correctsWeightEventId: randomUUID(),
        grams: "1",
        note: "No permission",
        idempotencyKey: randomUUID(),
      }),
  ...
  it("filters products within the current tenant, updates urgency atomically and exports decimal reports", async () => {
    ...
    await updateProductOperations(input);
    await updateProductOperations(input);
    await expect(
      updateProductOperations({ ...input, idempotencyKey: randomUUID() }),
    ).rejects.toMatchObject({ code: "CONFLICT" });
    const list = await listOperationalProducts({
      delayed: "true",
      urgent: "true",
    });
    ...
    expect(csv).toContain('"0.290"');
    expect(csv).toContain('"0.040"');
    expect(
      await prisma.auditLog.count({
        where: { organizationId: a.id, action: "report.exported" },
      }),
    ).toBe(1);
    session(foreign);
    expect((await listOperationalProducts({ q: product.barcode })).total).toBe(
      0,
    );
    session(viewer);
    await expect(exportProductReport("material", {})).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(
      listOperationalProducts({ issues: "true" }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  it("audits print/reprint, replays duplicate requests and prevents print permission bypass", async () => {
    ...
    const [first, second] = await Promise.all([
      prepareBarcodePrint(input),
      prepareBarcodePrint(input),
    ]);
    expect(second).toEqual(first);
    expect(first.barcode).toBe(product.barcode);
    expect(
      await prisma.auditLog.count({
        where: { organizationId: a.id, action: "barcode.print_requested" },
      }),
    ).toBe(1);
    await expect(
      prepareBarcodePrint({ ...input, idempotencyKey: randomUUID() }),
    ).rejects.toMatchObject({ code: "CONFLICT" });
    await prepareBarcodePrint({
      ...input,
      reprint: true,
      idempotencyKey: randomUUID(),
    });
    ...
    session(viewer);
    await expect(prepareBarcodePrint(input)).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
  });
  it("allows only a safe tenant-scoped location transfer with one history event", async () => {
    const location = await prisma.location.create({
      data: { organizationId: a.id, code: "SAFE", name: "Safe", type: "SAFE" },
    });
    ...
    await expect(
      updateProductOperations({ ...input, locationId: otherLocation.id }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await Promise.all([
      updateProductOperations(input),
      updateProductOperations(input),
    ]);
    expect(
  ...
  it("enforces an atomic shared rate limit under concurrent requests", async () => {
    ...
    const results = await Promise.allSettled(
      Array.from({ length: 8 }, () =>
        consumeRateLimit("test", key, 3, 60000, a.id, now),
      ),
    );
