...
if (!["localhost", "127.0.0.1"].includes(url.hostname))
  throw new Error("This helper is for the local Docker database only.");
...
function docker(args, input) {
  ...
  if (result.status !== 0)
    throw new Error(
      "Local database backup/restore command failed: " +
...
try {
  const backup = await database.$transaction(
    async (transaction) => {
      const [snapshot] =
        await transaction.$queryRaw`SELECT pg_export_snapshot() AS id`;
      ...
      const data = docker([
        "sh",
        "-c",
        'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" --format=custom --no-owner --schema="$2" --snapshot="$1"',
        "sh",
        snapshot.id,
        schema,
      ]);
  ...
  const manifest = {
    ...
    schema,
    sha256: createHash("sha256").update(backup.data).digest("hex"),
  ...
  if (process.argv.includes("--verify-restore")) {
    ...
    const restored = new PrismaClient({
      datasourceUrl: restoredUrl.toString(),
    });
    try {
      const [counts] = await restored.$queryRaw`SELECT
        ...
        (SELECT count(*)::int FROM "WeightEvent") AS weights,
        (SELECT count(*)::int FROM "Issue") AS issues`;
      if (JSON.stringify(counts) !== JSON.stringify(backup.counts))
        throw new Error(
          "Restore counts differ from the consistent backup snapshot.",
        );
