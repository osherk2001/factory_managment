    ...
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <header className="flex flex-wrap justify-between gap-4">
        ...
        <form action={logoutAction}>
          <button className="rounded border px-4 py-2">{m.app.logout}</button>
        </form>
      </header>
      {tenant ? (
        <p>
          {m.app.organization}: {tenant.organizationName}
        </p>
      ) : (
        <p>
          {memberships.length
      ...
      {memberships.length > 1 ? (
        <form
          ...
          className="flex flex-wrap gap-3 rounded-xl border bg-white p-5"
        >
          <label>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {links.map((l) => (
          <Link
            href={l.href}
            key={l.href}
            className="rounded-xl border bg-white p-6 text-lg font-medium shadow-sm hover:border-primary"
          >
            {l.label}
          </Link>
        ))}
      ...
      {user.isSystemAdmin ? (
        <Link
          href="/app/platform"
          className="inline-block rounded bg-primary px-5 py-3 text-primary-foreground"
        >
          {m.operations.platform}
        </Link>
      ) : null}
    </main>
  );
