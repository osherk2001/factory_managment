...
export default async function AuditPage({
  ...
  const parsed = z.coerce
    ...
    .min(1)
    .max(100000)
    .safeParse((await searchParams).page ?? 1);
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6 px-4 py-8">
      <h1 className="text-3xl font-semibold">{m.audit}</h1>
      <ol className="space-y-3">
        {records.slice(0, 50).map((r) => (
          <li
            key={r.id}
            className="space-y-1 rounded border bg-white p-4 text-sm"
          >
            <p className="font-medium">{r.action}</p>
            <p>
              {r.actorUser?.username} · {r.occurredAt.toLocaleString(locale)}
            </p>
      ...
      <nav className="flex gap-5">
        {page > 1 ? <Link href={`?page=${page - 1}`}>{m.previous}</Link> : null}
        {records.length > 50 ? (
          <Link href={`?page=${page + 1}`}>{m.next}</Link>
        ) : null}
      </nav>
    </main>
  );
