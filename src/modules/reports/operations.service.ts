...
export function productWhere(
  ...
  const active: Prisma.EnumProductStatusFilter = {
    in: ["CREATED", "IN_PROGRESS", "READY_FOR_HANDOFF"],
  };
  return {
    organizationId,
    status: filters.status ?? { not: "TRASHED" },
    ...(filters.q
      ? {
          OR: [
    ...
    ...(filters.delayed
      ? { AND: [{ status: active }, { targetAt: { lt: now } }] }
      : {}),
    ...(filters.issues
      ? { issues: { some: { organizationId, status: "OPEN" } } }
      : {}),
...
export async function listOperationalProducts(input: unknown) {
  ...
  const [products, total, workers, roles, locations] =
    await prisma.$transaction(
      [
        prisma.product.findMany({
          ...
          orderBy: [
            { isUrgent: "desc" },
            { targetAt: { sort: "asc", nulls: "last" } },
            { id: "asc" },
          ],
          skip: (filters.page - 1) * 30,
          take: 30,
          select: {
            id: true,
            serialNumber: true,
  ...
  return {
    filters,
    total,
    workers: workers.map((w) => ({ id: w.id, name: w.displayName })),
    roles,
    locations,
    canReadIssues: permissions.has("issues.read"),
    canExport: permissions.has("reports.export"),
    canCreate: permissions.has("products.create"),
    products: products.map((p) => ({
      id: p.id,
      serialNumber: p.serialNumber,
      status: p.status,
      isUrgent: p.isUrgent,
      targetAt: p.targetAt?.toISOString() ?? null,
      delayed: Boolean(
        p.targetAt &&
        p.targetAt < new Date() &&
        ["CREATED", "IN_PROGRESS", "READY_FOR_HANDOFF"].includes(p.status),
      ),
      worker: p.currentWorker?.displayName ?? null,
      role: p.currentRole?.name ?? null,
      location: p.currentLocation?.name ?? null,
      order: p.productionOrder?.orderNumber ?? null,
      customer: p.productionOrder?.customer?.name ?? null,
...
export async function getOperationalDashboard() {
  ...
  const active = {
    ...scope,
    status: { in: ["CREATED", "IN_PROGRESS", "READY_FOR_HANDOFF"] as const },
  };
  const activeWhere: Prisma.ProductWhereInput = {
    ...active,
    status: { in: [...active.status.in] },
  };
  const counts = await prisma.product.groupBy({
    by: ["status"],
    orderBy: { status: "asc" },
    where: { ...scope, status: { not: "TRASHED" } },
    _count: { _all: true },
  });
  const [urgent, delayed, issues] = await prisma.$transaction([
    prisma.product.count({ where: { ...activeWhere, isUrgent: true } }),
    prisma.product.count({
      where: { ...activeWhere, targetAt: { lt: new Date() } },
    }),
    prisma.issue.count({
      where: {
        ...scope,
        status: "OPEN",
        product: { status: { not: "TRASHED" } },
      },
    }),
  ]);
  return {
    ...
    urgent,
    delayed,
    issues: permissions.has("issues.read") ? issues : null,
  };
}
