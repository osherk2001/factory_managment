...
export async function selectLocale(form: FormData) {
  ...
  if (!locale.success)
    throw new ApplicationError("INVALID_INPUT", "Invalid locale");
  const session = await auth();
  if (session?.user?.id) {
    const user = await requireAuthenticatedUser();
    await prisma.user.update({
      where: { id: user.userId },
      data: { preferredLocale: locale.data },
    });
  }
  (await cookies()).set("factoryflow-locale", locale.data, {
    httpOnly: true,
...
export async function selectOrganization(form: FormData) {
  ...
  if (!id.success)
    throw new ApplicationError("INVALID_INPUT", "Invalid organization");
  await prisma.$transaction(async (database) => {
    const member = await database.membership.findFirst({
      where: { userId: user.userId, organizationId: id.data, status: "ACTIVE" },
      select: { id: true },
    });
    if (!member)
      throw new ApplicationError("FORBIDDEN", "Organization unavailable");
    await database.user.update({
      where: { id: user.userId },
      data: { selectedOrganizationId: id.data },
    });
  });
